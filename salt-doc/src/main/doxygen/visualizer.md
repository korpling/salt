...
How already mentioned, we also can instantiate the visualizer from an SDocument. The following example code demonstrate such a case. We use the sample document, generated by [SampleGenerator](@ref org.corpus_tools.salt.samples.SampleGenerator), which is described [here](#sample).
