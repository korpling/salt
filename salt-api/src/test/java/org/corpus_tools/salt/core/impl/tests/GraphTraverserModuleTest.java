...
public class GraphTraverserModuleTest {
  ...
  /**
   * This class checks if a traversing uses the predefined way. The predefined way can be set via
   * the variables nodeOrderWayThere and nodeOrderWayBack. This class implements the interface
   * {@link GraphTraverseHandler} and therefore listens for callbacks of the graph traversal.
   * 
  ...
  class TraverserChecker implements GraphTraverseHandler, Runnable {
    ...
    /**
     * determines if current run is cyclesafe. If this value is true, each cycle will be traversed
     * two times, than the method
     * {@link #checkConstraint(GRAPH_TRAVERSE_TYPE, String, Edge, Node, long)} will return false;
     */
    public boolean isCycleSafe = true;

    /**
     * The names of the nodes in order of how they shall be visited in current traversal mode on the
     * way there.
     */
    ...
    public void nodeReached(GRAPH_TRAVERSE_TYPE traversalType, String traversalId, SNode currNode,
        SRelation<SNode, SNode> rel, SNode fromNode, long order) {
      ...
      if (!currNode.getId().equalsIgnoreCase(nodeOrderWayThere[posInWayThere])) {
        ...
        throw new RuntimeException(
            "Test fails, another way than expected has been traversed on way there. Expected way was '"
                ...
                + "' at position '" + posInWayThere + "', node '" + currNode.getId()
                + "' was found.");
      }
      posInWayThere++;
    }
      ...
      if (!currNode.getId().equalsIgnoreCase(nodeOrderWayBack[posInWayBack])) {
        ...
        throw new RuntimeException(
            "Test fails, another way than expected has been traversed on way back. Expected way was '"
                + path + "', but instead of node '" + nodeOrderWayBack[posInWayBack]
                + "' at position '" + (posInWayBack + 1) + "', node '" + currNode.getId()
                + "' was found.");
      }
      posInWayBack++;
    }
    ...
    public boolean checkConstraint(GRAPH_TRAVERSE_TYPE traversalType, String traversalId,
        SRelation<SNode, SNode> rel, SNode currNode, long order) {
      boolean retVal = true;
  ...
  /**
   * Tests the method {@link GraphTraverserModule#getRoots()} and checks if the correct roots are
   * returned, using the several graph types.
   */
  @Test
  ...
  /**
   * Tests the method {@link GraphTraverserModule#getLeafs()} and checks if the correct leafs are
   * returned, using the several graph types.
   */
  @Test
  ...
  // traverseType = GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST;
  //
  // String[] nodeOrderWayThere = { "node1", "node2", "node3", "node6",
  // "node4", "node5", "node7" };
  // String[] nodeOrderWayBack = { "node3", "node6", "node2", "node5",
  // "node4", "node7", "node1" };
  ...
  /**
   * Starts 3 threads to traverse the graph and checks, that no problem concerning the use of
   * threads occurs. Uses {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_DEPTH_FIRST} as traverse order.
   * 
   * @throws Exception
   */
  @Test
  public void testThreading_TOP_DOWN_DEPTH_FIRST() throws Exception {
    ...
    while ((traverseChecker1.runs < 1) || (traverseChecker2.runs < 1)
        || (traverseChecker3.runs < 1)) {
      Thread.sleep(100);
    }

    List<TraverserChecker> traverseCheckers =
        new ArrayList<GraphTraverserModuleTest.TraverserChecker>();
  ...
  /**
   * Starts 3 threads to traverse the graph and checks, that no problem concerning the use of
   * threads occurs. Uses {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_BREADTH_FIRST} as traverse order.
   * 
   * @throws Exception
   */
  @Test
  ...
  public void testTraverse_TOP_DOWN_DEPTH_FIRST_Tree() {
    ...
    checker.nodeOrderWayBack = nodeOrderWayBack;
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getRoots(),
        GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST, "test_TOP_DOWN_DEPTH_FIRST_Tree", checker);
  ...
  public void testTraverse_TOP_DOWN_DEPTH_FIRST_DAG() {
    ...
    String[] nodeOrderWayThere =
        {"node1", "node2", "node3", "node6", "node4", "node2", "node3", "node6"};
    String[] nodeOrderWayBack =
        {"node3", "node6", "node2", "node1", "node3", "node6", "node2", "node4"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getRoots(),
        GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST, "test_TOP_DOWN_DEPTH_FIRST_DAG", checker);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }
  ...
  public void testTraverse_TOP_DOWN_DEPTH_FIRST_Cycle() throws Exception {
    ...
    String[] nodeOrderWayThere =
        {"node1", "node2", "node3", "node6", "node7", "node2", "node6", "node7", "node2"};
    String[] nodeOrderWayBack =
        {"node3", "node6", "node2", "node1", "node3", "node6", "node2", "node4"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    ...
    try {
      getFixture().traverse(getFixture().getGraph().getRoots(),
          GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST, "test_TOP_DOWN_DEPTH_FIRST_Cycle", checker);
      assertTrue(checker.checkNumberOfTraversedNodes());
  ...
  public void testTraverse_TOP_DOWN_DEPTH_FIRST_CycleUnsafe() {
    ...
    String[] nodeOrderWayThere =
        {"node1", "node2", "node3", "node6", "node7", "node2", "node3", "node6", "node7"};
    String[] nodeOrderWayBack =
        {"node3", "node3", "node7", "node6", "node2", "node7", "node6", "node2", "node1"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
  ...
  public void testTraverse_BOTTOM_UP_DEPTH_FIRST_Tree() {
    SGraph graph = TestHelper.createGraph_Tree();
    String[] nodeOrderWayThere = {"node3", "node2", "node1", "node5", "node4", "node1", "node6",
        "node2", "node1", "node7", "node1"};
    String[] nodeOrderWayBack = {"node1", "node2", "node3", "node1", "node4", "node5", "node1",
        "node2", "node6", "node1", "node7"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getLeafs(),
        GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST, "test_BOTTOM_UP_DEPTH_FIRST_Tree", checker);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }
  ...
  public void testTraverse_BOTTOM_UP_DEPTH_FIRST_DAG() {
    SGraph graph = TestHelper.createGraph_DAG();
    String[] nodeOrderWayThere =
        {"node3", "node2", "node1", "node4", "node6", "node2", "node1", "node4"};
    String[] nodeOrderWayBack =
        {"node1", "node4", "node2", "node3", "node1", "node4", "node2", "node6"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getLeafs(),
        GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST, "test_BOTTOM_UP_DEPTH_FIRST_DAG", checker);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }
  ...
  public void testTraverse_BOTTOM_UP_DEPTH_FIRST_Cycle() {
    SGraph graph = TestHelper.createGraph_Cycle();
    String[] nodeOrderWayThere =
        {"node3", "node2", "node1", "node4", "node7", "node6", "node2", "node1", "node4"};
    String[] nodeOrderWayBack =
        {"node1", "node4", "node2", "node3", "node1", "node4", "node2", "node6"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    ...
    try {
      getFixture().traverse(getFixture().getGraph().getLeafs(),
          GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST, "test_BOTTOM_UP_DEPTH_FIRST_Cycle", checker);
      assertTrue(checker.checkNumberOfTraversedNodes());
      fail("The graph contains a cycle, that shall invoke an exception.");
    } catch (Exception e) {
      ;
  ...
  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#BOTTOM_UP_DEPTH_FIRST} with a graph
   * containing a cycle, but with the "offending" edge is excluded by the filter.
   */
  @Test
  public void testTraverse_BOTTOM_UP_DEPTH_FIRST_NO_REAL_CYCLES() {
    ...
    final List<String> reachedNodes = new LinkedList<>();
    final List<String> leftNodes = new LinkedList<>();
    ...
    GraphTraverseHandler graphTraverseHandler = new GraphTraverseHandler() {
      ...
      public void nodeReached(GRAPH_TRAVERSE_TYPE traversalType, String traversalId, SNode currNode,
        ...
        reachedNodes.add(currNode.getName());
      }

      @Override
      public void nodeLeft(GRAPH_TRAVERSE_TYPE traversalType, String traversalId, SNode currNode,
          SRelation<SNode, SNode> relation, SNode fromNode, long order) {
        leftNodes.add(currNode.getName());
      }
    ...
    startNode.add(node3);
    getFixture().traverse(startNode, GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST,
        "testTraverse_BOTTOM_UP_DEPTH_FIRST_NO_REAL_CYCLES", graphTraverseHandler);
    assertThat(reachedNodes, CoreMatchers.is(Arrays.asList("node3", "node2", "node1")));
    assertThat(leftNodes, CoreMatchers.is(Arrays.asList("node1", "node2", "node3")));
  }


  /**
   * Starts 3 threads to traverse the graph and checks, that no problem concerning the use of
   * threads occurs. Uses {@link GRAPH_TRAVERSE_TYPE#BOTTOM_UP_DEPTH_FIRST} as traverse order.
   * 
   * @throws Exception
   */
  @Test
  ...
  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_BREADTH_FIRST} of graph_Tree, with
  ...
  public void testTraverse_TOP_DOWN_BREADTH_FIRST_Tree() {
    ...
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getRoots(),
        GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST, "test_TOP_DOWN_BREADTH_FIRST_Tree", checker,
        false);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }

  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_BREADTH_FIRST} of graph_DAG, with
  ...
  public void testTraverse_TOP_DOWN_BREADTH_FIRST_DAG() {
    SGraph graph = TestHelper.createGraph_DAG();
    String[] nodeOrderWayThere =
        {"node1", "node4", "node2", "node2", "node3", "node6", "node3", "node6"};
    String[] nodeOrderWayBack =
        {"node1", "node4", "node2", "node2", "node3", "node6", "node3", "node6"};
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    ...
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getRoots(),
        GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST, "test_TOP_DOWN_BREADTH_FIRST_DAG", checker,
        false);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }
  ...
  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_BREADTH_FIRST} of graph_DAG, with
   * graph {@inheritDoc SGraphTest#createGraph_DAG()}. Starting with a node that is not a root node
   */
  @Test
  public void testTraverse_TOP_DOWN_BREADTH_FIRST_DAG_NON_ROOT_START() {
    ...
    getFixture().traverse(startNodes, GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST,
  ...
  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#TOP_DOWN_DEPTH_FIRST} with a graph
   * containing a cycle, but with the "offending" edge is excluded by the filter.
   */
  @Test
  public void testTraverse_TOP_DOWN_DEPTH_FIRST_NO_REAL_CYCLES() {
    ...
    assertThat(reachedNodes, CoreMatchers.is(Arrays.asList("node1", "node2", "node3")));
    assertThat(leftNodes, CoreMatchers.is(Arrays.asList("node3", "node2", "node1")));
  ...
  public void testTraverse_TOP_DOWN_BREADTH_FIRST_CycleUnsafe() {
    ...
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getRoots(),
        GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST, "test_TOP_DOWN_BREADTH_FIRST_Cycle", checker,
        false);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }

  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#BOTTOM_UP_BREADTH_FIRST} of graph_Tree, with
   * tree {@inheritDoc SGraphTest#createGraph_Tree()}.
   */
  @Test
  public void testTraverse_BOTTOM_UP_BREADTH_FIRST_Tree() {
    ...
    String[] nodeOrderWayThere = {"node3", "node5", "node6", "node7", "node2", "node4", "node2",
    ...
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getLeafs(),
        GRAPH_TRAVERSE_TYPE.BOTTOM_UP_BREADTH_FIRST, "test_BOTTOM_UP_BREADTH_FIRST_Tree", checker,
        false);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }

  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#BOTTOM_UP_BREADTH_FIRST} of graph_DAG, with
   * graph {@inheritDoc SGraphTest#createGraph_DAG()}.
   */
  @Test
  public void testTraverse_BOTTOM_UP_BREADTH_FIRST_DAG() {
    SGraph graph = TestHelper.createGraph_DAG();
    String[] nodeOrderWayThere =
        {"node3", "node6", "node2", "node2", "node1", "node4", "node1", "node4"};
    String[] nodeOrderWayBack = nodeOrderWayThere.clone();
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
    getFixture().setGraph(graph);
    getFixture().traverse(getFixture().getGraph().getLeafs(),
        GRAPH_TRAVERSE_TYPE.BOTTOM_UP_BREADTH_FIRST, "test_BOTTOM_UP_BREADTH_FIRST_DAG", checker,
        false);
    assertTrue(checker.checkNumberOfTraversedNodes());
  }

  /**
   * Tests the traversing of {@link GRAPH_TRAVERSE_TYPE#BOTTOM_UP_BREADTH_FIRST} of graph_Cycle,
   * with graph {@inheritDoc SGraphTest#createGraph_Cycle()}.
   */
  @Test
  public void testTraverse_BOTTOM_UP_BREADTH_FIRST_Cycle() {
    SGraph graph = TestHelper.createGraph_Cycle();
    {
      ...
      try {
        getFixture().traverse(getFixture().getGraph().getLeafs(),
            GRAPH_TRAVERSE_TYPE.BOTTOM_UP_BREADTH_FIRST, "test_BOTTOM_UP_BREADTH_FIRST_Cycle",
            checker);
        assertTrue(checker.checkNumberOfTraversedNodes());
        fail("The graph contains a cycle, that shall invoke an exception.");
  ...
  public void testTraverse_BOTTOM_UP_BREADTH_FIRST_CycleUnsafe() {
    ...
    startNodes.add(graph.getNode("node7"));
    String[] nodeOrderWayThere =
        {"node3", "node7", "node2", "node6", "node1", "node7", "node2", "node6", "node1"};
    String[] nodeOrderWayBack = nodeOrderWayThere.clone();
    TraverserChecker checker = new TraverserChecker();
    checker.nodeOrderWayThere = nodeOrderWayThere;
    checker.nodeOrderWayBack = nodeOrderWayBack;
