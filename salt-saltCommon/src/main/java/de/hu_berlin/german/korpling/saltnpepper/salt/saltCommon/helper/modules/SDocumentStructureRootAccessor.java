	...
	public Hashtable<String, EList<SNode>> getRootsBySRelationSType(Class<? extends SRelation> clazz)
	{
		...
		{//walk through relations
					...
					{// the current relation has a SType
						...
						if (parentRelation instanceof SPointingRelation){
							...
							{ // the parent relation does not have a SRelation, too.Thus, the current relation is no root.
								storeSType.put(SaltFactory.DEFAULT_STYPE, false);
							} 
							else 
							{// the parent relation has STypes. Thus, the current relation is a root.
								if (storeSType.get(SaltFactory.DEFAULT_STYPE)== null)
								{
									storeSType.put(SaltFactory.DEFAULT_STYPE, true);
								}
							}
							...
							if (parentRelation.getSTypes() == null)
							{ // the parent relation does not have a SRelation, too.Thus, the current relation is no root.
								storeSType.put(SaltFactory.DEFAULT_STYPE_SORDERRELATION, false);
							} 
							else 
							{// the parent relation has STypes. Thus, the current relation is a root.
								if (storeSType.get(SaltFactory.DEFAULT_STYPE_SORDERRELATION)== null)
								{
									storeSType.put(SaltFactory.DEFAULT_STYPE_SORDERRELATION, true);
								}
							}
						}
			...
			if (storeSType.size()> 0)
			{
				...
				{
					if (currentRel instanceof SPointingRelation && storeSType.get(SaltFactory.DEFAULT_STYPE))
					{
						this.storeSType2SNode(SaltFactory.DEFAULT_STYPE, currentRel.getSSource());
					}
					else 
					if (currentRel instanceof SOrderRelation && storeSType.get(SaltFactory.DEFAULT_STYPE_SORDERRELATION))
					{
						this.storeSType2SNode(SaltFactory.DEFAULT_STYPE_SORDERRELATION, currentRel.getSSource());
					}
