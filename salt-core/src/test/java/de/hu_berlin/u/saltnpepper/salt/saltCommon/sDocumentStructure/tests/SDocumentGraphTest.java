...
public class SDocumentGraphTest {
	...
	public void testGetSTextualRelations() {
		...
		List<STextualRelation> textRels = new ArrayList<STextualRelation>();
		for (String id : ids) {
			STextualRelation textRel = SaltFactory.createSTextualRelation();
			textRel.setId(id);
			getFixture().addRelation(textRel);
			textRels.add(textRel);
		}
		assertTrue(textRels.containsAll(getFixture().getTextualRelations()));
		assertTrue(getFixture().getTextualRelations().containsAll(textRels));
	}
...
//	}
	...
	public void testGetSDominanceRelations() {
		List<SDominanceRelation> domRels = new ArrayList<SDominanceRelation>();
		for (int i = 0; i < 10; i++) {
			SDominanceRelation sDomRel = SaltFactory.createSDominanceRelation();
			domRels.add(sDomRel);
			getFixture().addRelation(sDomRel);
		}
		assertTrue(domRels.containsAll(getFixture().getDominanceRelations()));
		assertTrue(getFixture().getDominanceRelations().containsAll(domRels));
	}

	@Test
	public void testGetSPointingRelations() {
		List<SPointingRelation> pointingRels = new ArrayList<SPointingRelation>();
		for (int i = 0; i < 10; i++) {
			SPointingRelation sPointRel = SaltFactory.createSPointingRelation();
			pointingRels.add(sPointRel);
			getFixture().addRelation(sPointRel);
		}
		assertTrue(pointingRels.containsAll(getFixture().getPointingRelations()));
		assertTrue(getFixture().getPointingRelations().containsAll(pointingRels));
	}
	...
	public void testGetSNodeBySequence__DataSourceSequence() {
		...
		STextualDS sText1 = getFixture().createSTextualDS(text1);
		dsSequence.setDataSource(sText1);

		SToken tok2 = getFixture().createSToken(sText1, 5, 7);
		tok2.setName("tok2");

		SToken tok6 = getFixture().createSToken(sText1, 21, 22);
		tok6.setName("tok6");

		SToken tok1 = getFixture().createSToken(sText1, 0, 4);
		tok1.setName("tok1");

		SToken tok4 = getFixture().createSToken(sText1, 10, 16);
		tok4.setName("tok4");

		SToken tok5 = getFixture().createSToken(sText1, 17, 21);
		tok5.setName("tok5");

		SToken tok3 = getFixture().createSToken(sText1, 8, 9);
		tok3.setName("tok3");
		// end: sTokens for sText1
		// start: sTokens for sText2
		STextualDS sText2 = getFixture().createSTextualDS(text2);
		dsSequence.setDataSource(sText2);

		SToken tok3_1 = getFixture().createSToken(sText2, 9, 13);
		tok3_1.setName("tok3_1");

		SToken tok2_1 = getFixture().createSToken(sText2, 2, 8);
		tok2_1.setName("tok2_1");

		SToken tok1_1 = getFixture().createSToken(sText2, 0, 1);
	...
	public void testGetOverlappedDSSequences__SNode_EList() {
		...
		dsSequence.setEnd(4);
		SToken tok1 = getFixture().createSToken(dsSequence);
		tok1.setName("tok1");

		dsSequence.setStart(5);
		dsSequence.setEnd(7);
		SToken tok2 = getFixture().createSToken(dsSequence);
		tok2.setName("tok2");

		dsSequence.setStart(8);
		dsSequence.setEnd(9);
		SToken tok3 = getFixture().createSToken(dsSequence);
		tok3.setName("tok3");

		dsSequence.setStart(10);
		dsSequence.setEnd(16);
		SToken tok4 = getFixture().createSToken(dsSequence);
		tok4.setName("tok4");

		dsSequence.setStart(17);
		dsSequence.setEnd(21);
		SToken tok5 = getFixture().createSToken(dsSequence);
		tok5.setName("tok5");

		dsSequence.setStart(21);
		dsSequence.setEnd(22);
		SToken tok6 = getFixture().createSToken(dsSequence);
		tok6.setName("tok6");
		...
		SSpan span1 = getFixture().createSSpan(overlappedTokens);
		span1.setName("span1");

		overlappedNodes = new ArrayList<SStructuredNode>();
		overlappedNodes.add(struct2);
		overlappedNodes.add(span1);
		SStructure struct1 = getFixture().createSStructure(overlappedNodes);
		struct1.setName("struct1");

		overlappedNodes = new ArrayList<SStructuredNode>();
		overlappedNodes.add(span1);
		overlappedNodes.add(tok5);
		SStructure struct3 = getFixture().createSStructure(overlappedNodes);
...
//		sequence.setStart(0);
//		sequence.setEnd(1);
//		SToken tok1 = getFixture().createSToken(sequence);
...
//		sequence.setStart(2);
//		sequence.setEnd(8);
//		SToken tok2 = getFixture().createSToken(sequence);
//
//		sequence.setStart(9);
//		sequence.setEnd(13);
//		SToken tok3 = getFixture().createSToken(sequence);
//
//		List<SToken> sTokens = new ArrayList<SToken>();
//		sTokens.add(tok1);
//		sTokens.add(tok2);
...
//	}
	...
	public void testCreateSToken__SSequentialDS_Integer_Integer() {
		...
		STextualDS primaryText = getFixture().createSTextualDS(sampleText);
		SToken tok1 = getFixture().createSToken(primaryText, 0, 4);
		SToken tok2 = getFixture().createSToken(primaryText, 5, 7);
		SToken tok3 = getFixture().createSToken(primaryText, 8, 9);
		SToken tok4 = getFixture().createSToken(primaryText, 10, 16);
		SToken tok5 = getFixture().createSToken(primaryText, 16, 17);
	...
	public void testTokenize2() {
		...
		STextualDS sText1 = getFixture().createSTextualDS(text);
	...
	public void testInsertSTokenAt__STextualDS_Integer_String_Boolean() {
		...
		SToken token = getFixture().insertSTokenAt(sTextualDS, 5, text, true);
		assertNotNull(token);
	...
	public void testInsertSTokensAt__STextualDS_Integer_EList_Boolean() {
		...
		List<SToken> tokens = getFixture().insertSTokensAt(sTextualDS, 5, text, true);
	...
	public void testGetOverlappedSTokens__SNode_EList() {
		...
		List<SToken> overlappedTokenList1 = fixture.getOverlappedSTokens(sSpan, typeList);
		...
		tokenList.add(tok4);
		List<SToken> overlappedTokenListWithPointing = fixture.getOverlappedSTokens(sSpan, typeList2);
		...
		List<SToken> overlappedTokenList2 = fixture.getOverlappedSTokens(sStructure, typeList);
		...
		List<SToken> overlappedTokenList3 = fixture.getOverlappedSTokens(sStructure2, typeList);
