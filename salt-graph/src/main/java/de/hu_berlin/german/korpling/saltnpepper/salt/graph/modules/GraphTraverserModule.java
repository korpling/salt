...
public class GraphTraverserModule extends GraphModule {
	...
	public void traverse(EList<Node> startNodes,
		...
		if (this.getGraph() == null)
			throw new GraphTraverserException(
					"Cannot start traversing graph, because the graph is null.");
		if ((startNodes == null) || (startNodes.size() == 0))
			throw new GraphTraverserException("Cannot start traversing graph '"
					+ this.getGraph().getId()
			...
			if (traverseIdTable.containsKey(traverseHandler)) {
				...
				if (ids.contains(traverseId)) {
					throw new GraphTraverserException(
							"Cannot start traversing graph '"
									+ this.getGraph().getId()
									+ "', because the traverse id '"
									+ traverseId
									+ "' is already registered for the given callback handler '"
									+ traverseHandler + "'.");
				} else
					ids.add(traverseId);
			} else {
		...
		if (traverser.getException() != null) {
			...
			throw new GraphTraverserException(
					"Traversal of graph produced an exception. ",
					traverser.getException());
		}
	}
	...
	class Traverser implements Runnable {
		...
		public void run() {
			...
			try {
				if (GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST
						.equals(this.traverseType)) {
								...
								if (currentEntry != null) {// way down
									if (isCycleSafe) {
										...
										if (visitedNodes
											...
											text.append(currentEntry.node
													.getId());
											throw new GraphTraverserException(
													"A cycle in graph '"
															+ graph.getId()
															+ "' has been detected, while traversing with type '"
															+ traverseType
															+ "'. The cycle has been detected when visiting node '"
															+ currentEntry.node
															+ "' while current path was '"
															+ text.toString()
															+ "'.");
										}
									}
									...
									if (peekEntry == null)
										...
										traverseHandler
												.nodeReached(
														...
														currentEntry.node,
														peekEntry.edge,
														(peekEntry.edge != null) ? peekEntry.edge
																.getSource()
																: null,
														peekEntry.order);
									if (nextChild != null) {
				...
				} else if (GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST
						.equals(this.traverseType)) {
					...
					for (Node startNode : startNodes) {
						...
						if (traverseHandler.checkConstraint(
								GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST,
								traverseId, null, startNode, 0l)) {
							this.currentNodePath.add(startNode);
							this.bottomUpDepthFirstRec(null, 0);
						}
					}
				} else if (GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST
						.equals(this.traverseType)) {
					...
					for (Node startNode : startNodes) {
						...
						this.currentNodePath = new BasicEList<Node>();
						if (traverseHandler.checkConstraint(
								GRAPH_TRAVERSE_TYPE.TOP_DOWN_BREADTH_FIRST,
								traverseId, null, startNode, 0l)) {
							this.currentNodePath.add(startNode);
						}
					}
					breadthFirst();
				} else if (GRAPH_TRAVERSE_TYPE.BOTTOM_UP_BREADTH_FIRST
						.equals(this.traverseType)) {
					...
					for (Node startNode : startNodes) {
						...
						this.currentNodePath = new BasicEList<Node>();
						if (traverseHandler.checkConstraint(
								GRAPH_TRAVERSE_TYPE.BOTTOM_UP_BREADTH_FIRST,
								traverseId, null, startNode, 0l)) {
							this.currentNodePath.add(startNode);
						}
					}
				...
				if (e instanceof GraphTraverserException) {
					...
					ex = new GraphTraverserException(
							"An exception occured while traversing the graph '"
									+ graph.getId() + "' with path '"
									+ currentNodePath + "'.", e);
					this.setException(ex);
				}
			} finally {
		...
		private void breadthFirst() {
			if (isCycleSafe) {
				throw new GraphTraverserException(
						"Not able to detect cycles with breadth first search");
			}
			if ((this.currentNodePath == null)
					|| (this.currentNodePath.isEmpty()))
				throw new GraphTraverserException(
						"Cannot traverse node starting at empty start node.");
			...
			while (!queuedNodes.isEmpty()) {
				...
				EList<Edge> edgesOut = this.getGraph().getOutEdges(
						tNode.getId());
				EList<Edge> edges = null;
				currentNodePath.add(tNode);
				traverseHandler.nodeReached(traverseType, traverseId, tNode,
						fromEdge, fromNode, order);
				if (isTopDown) {
				...
				if (edges != null) {
					...
					for (Edge e : edges) {
						...
						if (traverseHandler.checkConstraint(traverseType,
								traverseId, e, n, order)) {
							queuedNodes.add(n);
				...
				traverseHandler.nodeLeft(traverseType, traverseId, tNode,
						fromEdge, fromNode, order);
		 ...
		 *            the parent node
		 */
		private void topDownDepthFirstRec(Edge edge, long order) {
			if ((this.currentNodePath == null)
					|| (this.currentNodePath.size() == 0))
				throw new GraphTraverserException(
						"Cannot traverse node starting at empty start node.");
			// current node is last element in currentPath
			Node currNode = this.currentNodePath.get(this.currentNodePath
					.size() - 1);
			Node parent = null;
			if (this.currentNodePath.size() > 1) {
				// if current path is larger then 1, than a parent node exists
				parent = this.currentNodePath
						.get(this.currentNodePath.size() - 2);
			}
			traverseHandler.nodeReached(
					GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST, traverseId,
					currNode, edge, parent, order);
			// walk through all childs of this node
			EList<Edge> childEdges = this.getGraph().getOutEdges(
					currNode.getId());
			if (childEdges != null) {
				// in case of node has childs
				int i = 0;
				for (Edge childEdge : childEdges) {
					...
					if (traverseHandler.checkConstraint(
							GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST,
							traverseId, childEdge, childNode, order)) {
						if ((this.isCycleSafe)
						...
						this.currentNodePath
								.remove(this.currentNodePath.size() - 1);
			...
			traverseHandler.nodeLeft(GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST,
					traverseId, currNode, edge, parent, order);
		}
		 ...
		 *            the child node
		 */
		private void bottomUpDepthFirstRec(Edge edge, long order) {
			if ((this.currentNodePath == null)
					|| (this.currentNodePath.size() == 0))
				throw new GraphTraverserException(
						"Cannot traverse node starting at empty start node.");
			...
			if (this.currentNodePath.size() > 1) {
				// if current path is larger then 1, than a parent node exists
				child = this.currentNodePath
						.get(this.currentNodePath.size() - 1);
			}

			traverseHandler.nodeReached(
					GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST, traverseId,
					currNode, edge, child, order);

			// walk through all childs of this node
			EList<Edge> parentEdges = this.getGraph().getInEdges(
					currNode.getId());
			if (parentEdges != null) {
				...
				for (Edge parentEdge : parentEdges) {
					...
					if (traverseHandler.checkConstraint(
							GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST,
							traverseId, parentEdge, parentNode, order)) {
			...
			traverseHandler.nodeLeft(GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST,
					traverseId, currNode, edge, child, order);
		}
	}
}
