...
//					this.cyclesLevel1= new BasicEList<List<SNode>>();
				...
				{//if some roots exists, start with them	
					...
					travObj.start((EList<Node>) (EList<? extends Object>) rootNodes);
					...
					for (Exception e:  travObj.getExceptions())
					{
						...
						throw(new SaltCoreModuleException("An exception occurs while computing cycles of graph '"+this.getSGraph().getSId()+".", e));
					}
				}//if some roots exists, start with them
				...
				{//check if all nodes have been traversed
					...
					{//do until all nodes has been visited
						...
						if (numOfNotVisitedRelations== this.notVisitedRelations.size())
							throw new SaltCoreModuleException("An exception occurs while computing cycles for subgraphs with no roots of graph '"+this.getSGraph().getSId()+"'. After computing cycles the number of not visited nodes is the same as before ("+numOfNotVisitedRelations+" has not been visited).");
					}//do until all nodes has been visited
				}//check if all nodes have been traversed
			}//if graph contains nodes and edges
		}//if a listener is given
...
//		}//debug
	...
	/**
	 * Stores the nodes, which has not been visited for second check
	 */
...
//						this.listener.cycleDetected(CYCLE_LEVELS.LEVEL_2, sRelation.getClass(), null, this.getShortCycle(slot, sNode));
				...
				{//if the last element of slot is not the source of current relation, add the source of current relation (this have to be done, because of it is missing otherwise)
					if (	(slot.size()== 0) ||
